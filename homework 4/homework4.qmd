...
```{r}
# the method of using pivot_longer function to convert w_ace and l_ace to "Winner" and "Loser" in a new column "role"
library(dplyr)
...
aces_data %>% group_by(role) %>% summarise(mean_aces = mean(aces), sd_aces = sd(aces))
```
...
player_stats <- player %>%
  left_join(winners, join_by("player_id" == "winner_id")) %>%
  mutate(wins = replace_na(total_win, 0),
