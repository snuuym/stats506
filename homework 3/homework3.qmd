...
The link to my github repository: [homework 3](https://github.com/snuuym/stats506/tree/main/homework%203)
...
library(pscl)  
library(dplyr)
#the idea of using midpoint to create the continuous income variable is inspired by chatgpt
income_map <- c(
  "$0 to $4,999"       = 2500,
  "$5,000 to $9,999"   = 7500,
  ...
  "$35,000 to $44,999" = 40000,
  "$45,000 to $54,999" = 50000,
  "$55,000 to $64,999" = 60000,
  "$65,000 to $74,999" = 70000,
  "$75,000 to $99,999" = 87500,
  "$100,000 and Over"  = 125000
)

df_clean$conti_income <- as.numeric(income_map[as.character(df_clean$INDHHIN2)])
...
#2R
model_2 <- glm(AUXTWIDR~RIAGENDR+ DMDCITZN + DMDHHSZA + conti_income, data = df_clean, family = "poisson",na.action = na.omit)
#1L
model_3 <- glm(AUXTWIDL~RIAGENDR, data = df_clean, family = "poisson",na.action = na.omit)
#2L
model_4 <- glm(AUXTWIDL~RIAGENDR+ DMDCITZN + DMDHHSZA + conti_income, data = df_clean, family = "poisson",na.action = na.omit)
...
exp(coef(model_4)["RIAGENDRFemale"])

model_no_gender <- glm(AUXTWIDL ~ DMDCITZN + DMDHHSZA + conti_income,
                       data = df_clean, family = "poisson", na.action = na.omit)
anova(model_no_gender, model_4, test = "Chisq")

```
*Answer:* The irr shows that females have an estimated 1.083238% higher tympanometric width in the left ear than males, and the following anova test prove that the predicted value of Tympanometric width measure of the left ear differs between men and women.
...
microbenchmark(
  ...
  sql_way = dbGetQuery(sakila, query),
  times = 50
)
...
leading <- as.numeric(substr(as.character(apt$apt_num), 1, 1))
